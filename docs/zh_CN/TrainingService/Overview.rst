...
用户可以使用 NNI 提供的训练平台来跑 trial, 训练平台有：`local machine <./LocalMode.md>`__\ ， `remote machines <./RemoteMachineMode.md>`__\ ， and on clusters like `PAI <./PaiMode.md>`__\ ，`Kubeflow <./KubeflowMode.md>`__\ ，`AdaptDL <./AdaptDLMode.md>`__\ ， `FrameworkController <./FrameworkControllerMode.md>`__\ ， `DLTS <./DLTSMode.md>`__ and `AML <./AMLMode.rst>`__。 这些都是 *内置的训练平台*。
